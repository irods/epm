					...
					/* I - Subpackage */
{
  ...
  // TGR for graphical installers
  FILE        *pis;                         /* pointer for Installed-Size */
  const int   isbuffersize = 1024;          /* buffer to hold Installed-Size */
  char        installedsize[isbuffersize];  /* Installed-Size for control file */
  char        iscmd[1024];                  /* command to compute Installed-Size */
  char        sedcmd[1024];                 /* command to replace the size placeholder */
  char        controlfilename[1024];        /* control filename */
  // TGR
  ...
  */
  ...
  snprintf(iscmd, sizeof(iscmd), "du -sx %s | awk '{print $1}'", filename); // TGR for Installed-Size
  mkdir(filename, 0777);
  ...
  strcpy(controlfilename, filename);
  ...
  fprintf(fp, "Installed-Size: 0\n"); // placeholder until the files are in place
  // TGR - end
  ...
  */
  ...
  // TGR - calculate and substitute installed size for graphical .deb installers
  pis = popen(iscmd, "r");
  if (fgets(installedsize, isbuffersize, pis) == NULL ) {
    fprintf(stderr, "fgets failed - du failed to get size of directory");
    exit(1);
  }
  pclose(pis);
  if (installedsize[strlen(installedsize)-1] == '\n') {
    installedsize[strlen(installedsize)-1] = '\0';
  }
  snprintf(sedcmd, sizeof(sedcmd), "sed -e 's/Installed-Size: 0/Installed-Size: %s/' %s > /tmp/epmcontrolfile", installedsize, controlfilename);
  pis = popen(sedcmd, "r");
  pclose(pis);
  snprintf(sedcmd, sizeof(sedcmd), "mv /tmp/epmcontrolfile %s", controlfilename);
  pis = popen(sedcmd, "r");
  pclose(pis);
  // TGR
